...
create table admin (admin_id char(6) primary key,
				    admin_password int(10),
                    admin_email varchar(200));
                    
create table actuators (actuator_id char(4) unique,
                        room_id char(4),
...
create table sensors (sensor_id char(4) primary key,
                      actuator_id char(4),
                      CONSTRAINT sensors_fk1 foreign key (actuator_id)
                      references actuators(actuator_id),
                      sensor_value decimal(10,3),
                      sensor_name varchar(50),
                      sensor_unit varchar(10));
			
create table historydata (data_id char(4) primary key,
                         sensor_id char(4),
